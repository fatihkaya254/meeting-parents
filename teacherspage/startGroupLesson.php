...
if (isset($_POST['startLesson']) && $_POST['startLesson'] == '1') {
    ...
    foreach ($groupStudents as $gs) {
        $wpdb->query("INSERT INTO `{$wpdb->prefix}mp_lesson_records` (`lr_id`, `student_id`, `teacher_id`, `hangiders`, `start_time`, `finish_time`, `lesson_status`, `homework_status`, `date_info`, `next_homework`, `branch_id`, `sms_ok`, `lesson_url`) VALUES (NULL, '$gs', '$teaid', '$hangisaat', '$time', '00:00:00', '0', '0', '$date', '0', '$branch', '0', NULL)");
    }

    for ($i = 0; $i < 6; $i++) {
        $wholeexams = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}mp_lesson_records WHERE student_id = '$groupStudents[$i]' AND teacher_id = '$teaid' AND hangiders = '$hangisaat' AND date_info = '$date';", ARRAY_A);
