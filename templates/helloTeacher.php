...
<style type="text/css">				
	...
	if ($dizi[0] == "T") {
		...
		for ($i=1; $i < 10 ; $i++) { 
			...
			$sty = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}mp_virtualweek WHERE vw_name = '$dizi[1] - $i. Saat' ;", ARRAY_A);
			foreach ($sty as $stu){
				if ($stu[$bugun] != "freeday" && $stu[$bugun] != "0") {
					...
					if ($stuid !="G") {
						...
						$record = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}mp_lesson_records WHERE student_id = '$stuid' AND date_info = '$tarih' AND branch_id = $braid AND teacher_id = '$dizi[1]' AND `hangiders` = '$i';", ARRAY_A);
						if ($record == null) {
							?>
							...
							<div class="content">
								...
								<form method="POST">
									<input type="hidden" name="hangihoca" value="<?php echo $dizi[1]; ?>">
										...
										if ($std4 !=0 ) {									
											...
											if ($std5 !=0 ) {
												...
												if ($std6 !=0 ) {
																			...
																			<div class="content">
																				...
																				foreach ($studentsall as $stdwjp) {
																					...
																					$theTopic = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}mp_lesson_records, {$wpdb->prefix}mp_student  WHERE {$wpdb->prefix}mp_lesson_records.student_id = {$wpdb->prefix}mp_student.student_id AND {$wpdb->prefix}mp_lesson_records.date_info ='$tarih' AND {$wpdb->prefix}mp_lesson_records.student_id = 
																						'$stdwjp' AND {$wpdb->prefix}mp_lesson_records.teacher_id = '$dizi[1]' AND {$wpdb->prefix}mp_lesson_records.hangiders = '$i';", ARRAY_A);
													...
													$birincisaat = $i.'1';
													$ikincisaat = $i.'2';
													$sty = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}mp_questionprocess, {$wpdb->prefix}mp_student WHERE {$wpdb->prefix}mp_questionprocess.qp_student_id = {$wpdb->prefix}mp_student.student_id AND {$wpdb->prefix}mp_questionprocess.qp_teacher_id = '$dizi[1]' AND {$wpdb->prefix}mp_questionprocess.qp_hour = '$birincisaat' AND {$wpdb->prefix}mp_questionprocess.qp_day = '$bugun' AND {$wpdb->prefix}mp_questionprocess.qp_teacher_id = '$dizi[1]' ;", ARRAY_A);
													foreach ($sty as $stu){
														$qpstudent_id = $stu['qp_student_id'];
														$record = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}mp_qprecords WHERE student_id = '$qpstudent_id' AND date_info = '$tarih' AND teacher_id = '$dizi[1]';", ARRAY_A);
														if ($record == null) {
															?>
															...
															$sty = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}mp_questionprocess, {$wpdb->prefix}mp_student WHERE {$wpdb->prefix}mp_questionprocess.qp_student_id = {$wpdb->prefix}mp_student.student_id AND {$wpdb->prefix}mp_questionprocess.qp_teacher_id = '$dizi[1]' AND {$wpdb->prefix}mp_questionprocess.qp_hour = '$ikincisaat' AND {$wpdb->prefix}mp_questionprocess.qp_day = '$bugun' AND {$wpdb->prefix}mp_questionprocess.qp_teacher_id = '$dizi[1]' ;", ARRAY_A);
															foreach ($sty as $stu){
																$qpstudent_id = $stu['qp_student_id'];
																$record = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}mp_qprecords WHERE student_id = '$qpstudent_id' AND date_info = '$tarih' AND teacher_id = '$dizi[1]';", ARRAY_A);
																if ($record == null) {
